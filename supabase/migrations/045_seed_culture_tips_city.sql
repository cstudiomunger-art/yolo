-- 045: Seed city-specific and universal culture tips
-- city_id = NULL  → universal (shown to all users with a saved itinerary)
-- city_id = 'xx'  → city-specific (shown when itinerary contains that city)
-- City-specific rows use SELECT ... WHERE EXISTS so they are silently skipped
-- if the referenced city does not yet exist in the cities table.

-- Universal tips (no city dependency)
...
SELECT
  ...
  20, TRUE, 'beijing'
WHERE EXISTS (SELECT 1 FROM cities WHERE id = 'beijing')
ON CONFLICT (id) DO UPDATE SET
  preview    = EXCLUDED.preview,
  body       = EXCLUDED.body,
  do_text    = EXCLUDED.do_text,
...
SELECT
  ...
  30, TRUE, 'shanghai'
WHERE EXISTS (SELECT 1 FROM cities WHERE id = 'shanghai')
ON CONFLICT (id) DO UPDATE SET
  preview    = EXCLUDED.preview,
  body       = EXCLUDED.body,
  do_text    = EXCLUDED.do_text,
...
SELECT
  ...
  40, TRUE, 'chengdu'
WHERE EXISTS (SELECT 1 FROM cities WHERE id = 'chengdu')
ON CONFLICT (id) DO UPDATE SET
  preview    = EXCLUDED.preview,
  body       = EXCLUDED.body,
  do_text    = EXCLUDED.do_text,
...
SELECT
  ...
  50, TRUE, 'xian'
WHERE EXISTS (SELECT 1 FROM cities WHERE id = 'xian')
ON CONFLICT (id) DO UPDATE SET
  preview    = EXCLUDED.preview,
  body       = EXCLUDED.body,
  do_text    = EXCLUDED.do_text,
