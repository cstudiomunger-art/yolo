...
/// Loads Info Hub content (transport tips, common phrases, dialect phrases) from
...
final class InfoHubContentService {
    ...
    private static let cacheKey = "yolohappy.cachedInfoHubContent.v1"
    ...
    func load() async {
        ...
        do {
            ...
            let loaded = try await InfoHubContent(transport: transport, common: common, dialect: dialect)
            if loaded.transport.isEmpty && loaded.common.isEmpty && loaded.dialect.isEmpty {
                content = cached() ?? Self.bundledFallback
            } else {
                content = loaded
                cache(loaded)
    ...
    // MARK: - Bundled fallback (mirrors migration 065 seed)
    ...
    static let bundledFallback: InfoHubContent = {
        ...
        return InfoHubContent(transport: transport, common: common, dialect: dialect)
    }()
}
